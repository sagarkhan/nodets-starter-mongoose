...
import { responseFormatter } from '../../../../common/middlewares/response.handler';
...
export class Controller {
  async find(): Promise<any> {
    try {
      ...
      throw new HttpErrors().Unprocessable('', err);
    }
  }
  ...
  async get(req: Request): Promise<any> {
    try {
      ...
      if (!result) {
        return new HttpErrors().NotFound('Record not found');
      }
      return responseFormatter(HttpStatus.OK, result);
    } catch (err) {
      throw new HttpErrors().Unprocessable('', err);
